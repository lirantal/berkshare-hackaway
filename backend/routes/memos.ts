...
memosRouter.post('/memos', (req, res) => {
    ...
    const baseContactInformation = db.prepare(`SELECT user_profile.email, user_profile.address, user_profile.phone_number FROM user_profile WHERE user_profile.user_id = ?`).get(res.locals.user.id);
    const contactInMemo = JSON.parse(meta);
    const userExtendedMeta = recursiveJSONMerge(baseContactInformation, contactInMemo);

    const date = new Date().toISOString();
    db.prepare('INSERT INTO memos (user_id, text, meta, date) VALUES (?, ?, ?, ?)').run(res.locals.user.id, text, JSON.stringify(userExtendedMeta), date);
